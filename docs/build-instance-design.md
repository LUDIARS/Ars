...
│  │  (main branch)│────▶│ c7g.xlarge   │────▶│  ars-build-artifacts│  │
│  │              │     │ (ARM/Graviton)│     │                    │  │
│  └──────────────┘     │              │     │  /builds/          │  │
│         │             │ - git pull    │     │    v0.1.0/         │  │
│         │             │ - npm ci      │     │      dist.tar.gz  │  │
...
| 項目 | 値 |
|------|------|
| インスタンスタイプ | **c7g.xlarge** (4 vCPU / 8GB RAM, Graviton3) ※GitHub Actions 変数で変更可能 |
...
- **c7g.xlarge** (推奨): 4 vCPU / 8GB。EBS 永続化により cargo/npm キャッシュが残るため、
  差分ビルドでは 4 コアで十分高速。Graviton3 ARM ネイティブで運用サーバ (t4g) と
  アーキテクチャ一致、クロスコンパイル不要
- 停止時はEBSストレージ料金 (月額約$2.40) のみ発生
- 起動時: $0.174/h (東京) × ビルド時間 (約5-10分) = **1回あたり約$0.01-0.03**

#### インスタンスタイプ比較 (東京リージョン参考)

GitHub Actions の `BUILD_INSTANCE_TYPE` 変数でインスタンスタイプを切り替え可能。
初回ビルドや大規模変更時のみ上位タイプに変更する運用を推奨。

| タイプ | vCPU | RAM | 時間単価 (東京概算) | 用途 |
|--------|------|-----|-------------------|------|
| c7g.large | 2 | 4GB | ~$0.087/h | 最小構成・軽微な差分ビルド |
| **c7g.xlarge** | **4** | **8GB** | **~$0.174/h** | **推奨: 通常ビルド** |
| c7g.2xlarge | 8 | 16GB | ~$0.348/h | 初回ビルド・フルリビルド |

> **注意**: t4g (バースト型) は持続的 CPU 使用でクレジット超過料金が発生するため、
> ビルド用途には c7g (コンピュート最適化) を推奨します。
...
## 5. コスト試算 (c7g.xlarge 東京リージョン)

| 項目 | 月額コスト |
|------|-----------|
| ビルドインスタンス (停止時 EBS) | ~$2.40 (30GB gp3) |
| ビルドインスタンス (起動時) | ~$0.174/h × 回数 |
| S3 ストレージ | ~$0.50 (数GB程度) |
| S3 転送 (同一リージョン) | $0.00 |
| **月10回ビルドの場合** | **~$3.20/月** |
| **月30回ビルドの場合** | **~$3.80/月** |
...
├── setup-iam.sh               # IAM ロール・ポリシー作成スクリプト
├── build.sh                   # ビルド実行スクリプト
...
## 8. GitHub Actions 変数一覧

リポジトリの Settings > Secrets and variables > Actions > Variables に設定する。

| 変数名 | 説明 | 例 |
|--------|------|-----|
| `BUILD_INSTANCE_ID` | ビルド EC2 インスタンス ID | `i-0123456789abcdef0` |
| `AWS_BUILD_ROLE_ARN` | ビルド用 IAM ロール ARN | `arn:aws:iam::123456789012:role/ars-github-actions-build` |
| `AWS_DEPLOY_ROLE_ARN` | デプロイ用 IAM ロール ARN | `arn:aws:iam::123456789012:role/ars-github-actions-deploy` |
| `PROD_INSTANCE_ID` | 運用サーバ EC2 インスタンス ID | `i-0abcdef1234567890` |
| `S3_BUCKET` | S3 バケット名 | `ars-build-artifacts` |
| `AWS_REGION` | AWS リージョン (省略時 ap-northeast-1) | `ap-northeast-1` |
