...
> **前提**: Ars が管理するプロジェクトは `codedesign/` / `gamedesign/` / `data/` / `test/` の 4 フォルダで構成される。本ルールは主に `codedesign/` 内の構造を定義する。`test/` の詳細は `test-rules.md` を参照。
...
└── test/                                   # テスト成果物（test-rules.md 参照）
    ...
    ├── integration/                        # シーン単位の統合テスト（AI骨子 + 人間詳細）
    ├── drivers/                            # 機能テスト用に自動生成されるドライバー
