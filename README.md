...
Ars が管理するプロジェクトは、以下の **4 つのフォルダ**で構成されます。
...
| `test/` | テスト成果物 — 機能テスト・統合テスト・ドライバー・テストモジュール（`spec/rule/test-rules.md` 参照） |

この 4 フォルダ構成は Ars の根幹概念であり、エディタ・コード生成・MCP Server のすべてがこの構造を前提に動作します。
